#!/usr/bin/env python3
"""
Map Tommy Carlsson case variation
Super-safe candidate: same club (Mjölner), no caution factors
Tommy Carlsson (17 matches) vs Tommy carlsson (35 matches)
"""
import sqlite3

def analyze_tommy_carlsson():
    """Analyze Tommy Carlsson vs Tommy carlsson before mapping"""
    print("=== ANALYZING TOMMY CARLSSON BEFORE MAPPING ===")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        # Get player IDs
        cursor.execute("SELECT id, name FROM players WHERE name LIKE 'Tommy %arlsson'")
...
def apply_tommy_mapping(player_mapping):
    """Apply Tommy Carlsson mapping"""
    ...
    for name, player_id in player_mapping.items():
        if name == "Tommy Carlsson":
            proper_case = player_id
        elif name == "Tommy carlsson":
            lower_case = player_id

    if not proper_case or not lower_case:
        print("ERROR: Could not find both Tommy variations")
        return False

    # Map lower case to proper case
    source_player_id = lower_case     # "Tommy carlsson"
    target_player_id = proper_case    # "Tommy Carlsson"
    canonical_name = "Tommy Carlsson"

    print(f"\n=== APPLYING MAPPING ===")
    print(f"Source: 'Tommy carlsson' (ID: {source_player_id})")
    print(f"Target: 'Tommy Carlsson' (ID: {target_player_id})")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        for sub_match in sub_matches:
            ...
            """, (
                ...
                "Super-safe candidate: Tommy carlsson -> Tommy Carlsson"
            ))
...
def main():
    print("=== TOMMY CARLSSON CASE VARIATION MAPPING ===")

    # Analyze
    player_mapping = analyze_tommy_carlsson()

    # Apply mapping
    success = apply_tommy_mapping(player_mapping)

    if success:
        print("\nTommy Carlsson mapping completed successfully!")
    else:
        print("\nTommy Carlsson mapping failed!")
