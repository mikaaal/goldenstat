#!/usr/bin/env python3
"""
Map Marcus Gavander case variation
Marcus Gavander vs Marcus gavander
"""
import sqlite3

def analyze_marcus_gavander():
    """Analyze Marcus Gavander vs Marcus gavander before mapping"""
    print("=== ANALYZING MARCUS GAVANDER BEFORE MAPPING ===")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        # Get player IDs
        cursor.execute("SELECT id, name FROM players WHERE name LIKE 'Marcus %avander'")
...
def apply_marcus_mapping(player_mapping):
    """Apply Marcus Gavander mapping"""
    ...
    for name, player_id in player_mapping.items():
        if name == "Marcus Gavander":
            proper_case = player_id
        elif name == "Marcus gavander":
            lower_case = player_id

    if not proper_case or not lower_case:
        print("ERROR: Could not find both Marcus variations")
        return False

    # Map lower case to proper case
    source_player_id = lower_case     # "Marcus gavander"
    target_player_id = proper_case    # "Marcus Gavander"
    canonical_name = "Marcus Gavander"

    print(f"\n=== APPLYING MAPPING ===")
    print(f"Source: 'Marcus gavander' (ID: {source_player_id})")
    print(f"Target: 'Marcus Gavander' (ID: {target_player_id})")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        for sub_match in sub_matches:
            ...
            """, (
                ...
                85,
                "Case variation: same club analysis pending",
                "Candidate: Marcus gavander -> Marcus Gavander"
            ))
...
def main():
    print("=== MARCUS GAVANDER CASE VARIATION MAPPING ===")

    # Analyze
    player_mapping = analyze_marcus_gavander()

    # Apply mapping
    success = apply_marcus_mapping(player_mapping)

    if success:
        print("\nMarcus Gavander mapping completed successfully!")
    else:
        print("\nMarcus Gavander mapping failed!")
