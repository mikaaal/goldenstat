#!/usr/bin/env python3
"""
Map Paulina Norman case variation
Nearly-safe candidate: same club (Steel Tons), 1 low/medium temporal conflict
Paulina Norman (38 matches) vs Paulina norman (36 matches)
"""
import sqlite3

def analyze_paulina_norman():
    """Analyze Paulina Norman vs Paulina norman before mapping"""
    print("=== ANALYZING PAULINA NORMAN BEFORE MAPPING ===")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        # Get player IDs
        cursor.execute("SELECT id, name FROM players WHERE name LIKE 'Paulina %orman'")
...
def apply_paulina_mapping(player_mapping):
    """Apply Paulina Norman mapping"""
    ...
    for name, player_id in player_mapping.items():
        if name == "Paulina Norman":
            proper_case = player_id
        elif name == "Paulina norman":
            lower_case = player_id

    if not proper_case or not lower_case:
        print("ERROR: Could not find both Paulina variations")
        return False

    # Map lower case to proper case
    source_player_id = lower_case     # "Paulina norman"
    target_player_id = proper_case    # "Paulina Norman"
    canonical_name = "Paulina Norman"

    print(f"\n=== APPLYING MAPPING ===")
    print(f"Source: 'Paulina norman' (ID: {source_player_id})")
    print(f"Target: 'Paulina Norman' (ID: {target_player_id})")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        for sub_match in sub_matches:
            ...
            """, (
                ...
                "Nearly-safe candidate: Paulina norman -> Paulina Norman"
            ))
...
def main():
    print("=== PAULINA NORMAN CASE VARIATION MAPPING ===")

    # Analyze
    player_mapping = analyze_paulina_norman()

    # Apply mapping
    success = apply_paulina_mapping(player_mapping)

    if success:
        print("\nPaulina Norman mapping completed successfully!")
    else:
        print("\nPaulina Norman mapping failed!")
