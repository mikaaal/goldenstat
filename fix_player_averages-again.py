...
Problem: Checkout throws have score=number of darts used, not points scored.
The actual points for checkout is the remaining_score before the checkout throw.
"""
...
def calculate_correct_average(db_path='goldenstat.db'):
    ...
    for sub_match_id in sub_match_ids:
        try:
            ...
            for throw in all_throws:
                leg_num, team, score, darts_used, remaining = throw
                key = (leg_num, team)

                if remaining == 0:
                    # Checkout throw: score = darts used, points = previous remaining
                    checkout_darts = score if score else 3
                    checkout_score = last_remaining.get(key, 0)
                    team_stats[team]['score'] += checkout_score
                    team_stats[team]['darts'] += checkout_darts
                elif score > 0:
                    # Regular throw: add score and darts
    ...
    conn.commit()
    print(f"✓ Updated {cursor.rowcount} player averages")

    # Verify with our test cases
    print("\n=== VERIFICATION ===")
    test_cases = [
