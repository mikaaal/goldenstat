#!/usr/bin/env python3
"""
Map Jocke Olsson case variation
Jocke Olsson (42 matches) vs Jocke olsson (24 matches)
"""
import sqlite3

def analyze_jocke_olsson():
    """Analyze Jocke Olsson vs Jocke olsson before mapping"""
    print("=== ANALYZING JOCKE OLSSON BEFORE MAPPING ===")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        # Get player IDs
        cursor.execute("SELECT id, name FROM players WHERE name LIKE 'Jocke %lsson'")
...
def apply_jocke_mapping(player_mapping):
    """Apply Jocke Olsson mapping"""
    ...
    for name, player_id in player_mapping.items():
        if name == "Jocke Olsson":
            proper_case = player_id
        elif name == "Jocke olsson":
            lower_case = player_id

    if not proper_case or not lower_case:
        print("ERROR: Could not find both Jocke variations")
        return False

    # Map lower case to proper case
    source_player_id = lower_case     # "Jocke olsson"
    target_player_id = proper_case    # "Jocke Olsson"
    canonical_name = "Jocke Olsson"

    print(f"\n=== APPLYING MAPPING ===")
    print(f"Source: 'Jocke olsson' (ID: {source_player_id})")
    print(f"Target: 'Jocke Olsson' (ID: {target_player_id})")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        for sub_match in sub_matches:
            ...
            """, (
                ...
                85,
                "Case variation: same club, temporal conflicts noted",
                "Candidate: Jocke olsson -> Jocke Olsson"
            ))
...
def main():
    print("=== JOCKE OLSSON CASE VARIATION MAPPING ===")

    # Analyze
    player_mapping = analyze_jocke_olsson()

    # Apply mapping
    success = apply_jocke_mapping(player_mapping)

    if success:
        print("\nJocke Olsson mapping completed successfully!")
    else:
        print("\nJocke Olsson mapping failed!")
