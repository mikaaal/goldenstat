#!/usr/bin/env python3
"""
Map Gareth Young case variation
Super-safe candidate: same club (Dartanjang), no caution factors
Gareth Young (24 matches) vs Gareth young (28 matches)
"""
import sqlite3

def analyze_gareth_young():
    """Analyze Gareth Young vs Gareth young before mapping"""
    print("=== ANALYZING GARETH YOUNG BEFORE MAPPING ===")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        # Get player IDs
        cursor.execute("SELECT id, name FROM players WHERE name LIKE 'Gareth %oung'")
...
def apply_gareth_mapping(player_mapping):
    """Apply Gareth Young mapping"""
    ...
    for name, player_id in player_mapping.items():
        if name == "Gareth Young":
            proper_case = player_id
        elif name == "Gareth young":
            lower_case = player_id

    if not proper_case or not lower_case:
        print("ERROR: Could not find both Gareth variations")
        return False

    # Map lower case to proper case
    source_player_id = lower_case     # "Gareth young"
    target_player_id = proper_case    # "Gareth Young"
    canonical_name = "Gareth Young"

    print(f"\n=== APPLYING MAPPING ===")
    print(f"Source: 'Gareth young' (ID: {source_player_id})")
    print(f"Target: 'Gareth Young' (ID: {target_player_id})")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        for sub_match in sub_matches:
            ...
            """, (
                ...
                "Super-safe candidate: Gareth young -> Gareth Young"
            ))
...
def main():
    print("=== GARETH YOUNG CASE VARIATION MAPPING ===")

    # Analyze
    player_mapping = analyze_gareth_young()

    # Apply mapping
    success = apply_gareth_mapping(player_mapping)

    if success:
        print("\nGareth Young mapping completed successfully!")
    else:
        print("\nGareth Young mapping failed!")
