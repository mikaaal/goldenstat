#!/usr/bin/env python3
"""
Map Niklas Pettersson case variation
Super-safe candidate: same club (Pilen), no caution factors
Niklas Pettersson (19 matches) vs Niklas pettersson (33 matches)
"""
import sqlite3

def analyze_niklas_pettersson():
    """Analyze Niklas Pettersson vs Niklas pettersson before mapping"""
    print("=== ANALYZING NIKLAS PETTERSSON BEFORE MAPPING ===")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        # Get player IDs
        cursor.execute("SELECT id, name FROM players WHERE name LIKE 'Niklas %ettersson'")
...
def apply_niklas_mapping(player_mapping):
    """Apply Niklas Pettersson mapping"""
    ...
    for name, player_id in player_mapping.items():
        if name == "Niklas Pettersson":
            proper_case = player_id
        elif name == "Niklas pettersson":
    ...
    source_player_id = lower_case     # "Niklas pettersson"
    target_player_id = proper_case    # "Niklas Pettersson"
    canonical_name = "Niklas Pettersson"

    print(f"\n=== APPLYING MAPPING ===")
    print(f"Source: 'Niklas pettersson' (ID: {source_player_id})")
    print(f"Target: 'Niklas Pettersson' (ID: {target_player_id})")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        for sub_match in sub_matches:
            ...
            """, (
                ...
                target_player_id,
                canonical_name,
                "Case variation mapping - Pilen",
                95,
                "Safe case variation: same club, no temporal conflicts",
                "Super-safe candidate: Niklas pettersson -> Niklas Pettersson"
            ))
...
def main():
    print("=== NIKLAS PETTERSSON CASE VARIATION MAPPING ===")

    # Analyze
    player_mapping = analyze_niklas_pettersson()

    # Apply mapping
    success = apply_niklas_mapping(player_mapping)

    if success:
        print("\nNiklas Pettersson mapping completed successfully!")
    else:
        print("\nNiklas Pettersson mapping failed!")
