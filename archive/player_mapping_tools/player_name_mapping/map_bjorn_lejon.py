#!/usr/bin/env python3
"""
Map Björn Lejon case variation
Nearly-safe candidate: same club (Bälsta), 1 low/medium temporal conflict
Björn Lejon (73 matches) vs Björn lejon (9 matches)
"""
import sqlite3

def analyze_bjorn_lejon():
    """Analyze Björn Lejon vs Björn lejon before mapping"""
    print("=== ANALYZING BJÖRN LEJON BEFORE MAPPING ===")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        cursor.execute("SELECT id, name FROM players WHERE name LIKE '%j_rn%ejon'")
...
def apply_bjorn_mapping(player_mapping):
    """Apply Björn Lejon mapping"""
    ...
    for name, player_id in player_mapping.items():
        if "Björn Lejon" in name:
            proper_case = player_id
        elif "Björn lejon" in name:
            lower_case = player_id

    if not proper_case or not lower_case:
        print("ERROR: Could not find both Björn variations")
        print("Available players:", list(player_mapping.keys()))
        return False

    # Map lower case to proper case
    source_player_id = lower_case     # "Björn lejon"
    target_player_id = proper_case    # "Björn Lejon"
    canonical_name = "Björn Lejon"

    print(f"\n=== APPLYING MAPPING ===")
    print(f"Source: 'Björn lejon' (ID: {source_player_id})")
    print(f"Target: 'Björn Lejon' (ID: {target_player_id})")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        for sub_match in sub_matches:
            ...
            """, (
                ...
                "Nearly-safe candidate: Björn lejon -> Björn Lejon"
            ))
...
def main():
    print("=== BJÖRN LEJON CASE VARIATION MAPPING ===")

    # Analyze
    player_mapping = analyze_bjorn_lejon()

    # Apply mapping
    success = apply_bjorn_mapping(player_mapping)

    if success:
        print("\nBjörn Lejon mapping completed successfully!")
    else:
        print("\nBjörn Lejon mapping failed!")
