#!/usr/bin/env python3
"""
Map Ulrika Landberg case variation
Ulrika Landberg vs Ulrika landberg
"""
import sqlite3

def analyze_ulrika_landberg():
    """Analyze Ulrika Landberg vs Ulrika landberg before mapping"""
    print("=== ANALYZING ULRIKA LANDBERG BEFORE MAPPING ===")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        # Get player IDs
        cursor.execute("SELECT id, name FROM players WHERE name LIKE 'Ulrika %andberg'")
...
def apply_ulrika_mapping(player_mapping):
    """Apply Ulrika Landberg mapping"""
    ...
    for name, player_id in player_mapping.items():
        if name == "Ulrika Landberg":
            proper_case = player_id
        elif name == "Ulrika landberg":
            lower_case = player_id

    if not proper_case or not lower_case:
        print("ERROR: Could not find both Ulrika variations")
        print("Available players:", list(player_mapping.keys()))
        return False

    # Map lower case to proper case
    source_player_id = lower_case     # "Ulrika landberg"
    target_player_id = proper_case    # "Ulrika Landberg"
    canonical_name = "Ulrika Landberg"

    print(f"\n=== APPLYING MAPPING ===")
    print(f"Source: 'Ulrika landberg' (ID: {source_player_id})")
    print(f"Target: 'Ulrika Landberg' (ID: {target_player_id})")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        for sub_match in sub_matches:
            ...
            """, (
                ...
                "Candidate: Ulrika landberg -> Ulrika Landberg"
            ))
...
def main():
    print("=== ULRIKA LANDBERG CASE VARIATION MAPPING ===")

    # Analyze
    player_mapping = analyze_ulrika_landberg()

    # Apply mapping
    success = apply_ulrika_mapping(player_mapping)

    if success:
        print("\nUlrika Landberg mapping completed successfully!")
    else:
        print("\nUlrika Landberg mapping failed!")
