#!/usr/bin/env python3
"""
Map Joop Van Gerwen case variation
Super-safe candidate: same club (Mjölner), no caution factors
Joop Van Gerwen (33 matches) vs Joop van Gerwen (30 matches)
"""
import sqlite3

def analyze_joop_van_gerwen():
    """Analyze Joop Van Gerwen vs Joop van Gerwen before mapping"""
    print("=== ANALYZING JOOP VAN GERWEN BEFORE MAPPING ===")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        # Get player IDs
        cursor.execute("SELECT id, name FROM players WHERE name LIKE 'Joop %an Gerwen'")
...
def apply_joop_mapping(player_mapping):
    """Apply Joop Van Gerwen mapping"""
    ...
    for name, player_id in player_mapping.items():
        if name == "Joop Van Gerwen":
            proper_case = player_id
        elif name == "Joop van Gerwen":
            lower_case = player_id

    if not proper_case or not lower_case:
        print("ERROR: Could not find both Joop variations")
        return False

    # Map lower case to proper case
    source_player_id = lower_case     # "Joop van Gerwen"
    target_player_id = proper_case    # "Joop Van Gerwen"
    canonical_name = "Joop Van Gerwen"

    print(f"\n=== APPLYING MAPPING ===")
    print(f"Source: 'Joop van Gerwen' (ID: {source_player_id})")
    print(f"Target: 'Joop Van Gerwen' (ID: {target_player_id})")
    ...
    with sqlite3.connect("../goldenstat.db") as conn:
        ...
        for sub_match in sub_matches:
            ...
            """, (
                ...
                "Super-safe candidate: Joop van Gerwen -> Joop Van Gerwen"
            ))
...
def main():
    print("=== JOOP VAN GERWEN CASE VARIATION MAPPING ===")

    # Analyze
    player_mapping = analyze_joop_van_gerwen()

    # Apply mapping
    success = apply_joop_mapping(player_mapping)

    if success:
        print("\nJoop Van Gerwen mapping completed successfully!")
    else:
        print("\nJoop Van Gerwen mapping failed!")
